...
#pragma warning(suppress: 6262)
DWORD WINAPI IntelliHostThread(LPVOID lpParam)
{
...
void StartProcessing()
{
	try
	{
		g_pServerSocket.CreateAndBind(IntelliHostPort, SOCK_STREAM, AF_INET);
		if (g_pServerSocket.IsCreated())
		{
			...
			while (true)
			{
				...
				g_hThreadArray[g_nThreadCount] = CreateThread(nullptr, 0, IntelliHostThread, (int*)&nSocketIndex, 0, &m_dwThreadID[g_nThreadCount]);
