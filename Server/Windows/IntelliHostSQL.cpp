...
{
public:
	TCHAR m_lpszFilepath[4000];  // File path (relative to IntelliHost root)
...
{
public:
	bool Iterate(const CODBC::CConnection& pDbConnect, CWSocket& pApplicationSocket, SHA256& pSHA256, _In_ bool bBind = true, _In_opt_ CODBC::SQL_ATTRIBUTE* pAttributes = nullptr, _In_ ULONG nAttributes = 0)
	{
#pragma warning(suppress: 26477)
		SQLRETURN nRet{ Open(pDbConnect, GetDefaultCommand(), bBind, pAttributes, nAttributes) };
		ODBC_CHECK_RETURN_FALSE(nRet, m_Command);
		...
		while (true)
		{
			...
			if (WriteBuffer(pApplicationSocket, (unsigned char*)decoded.data(), (int)decoded.length(), false, false))
			{
...
#pragma warning(suppress: 26472)
	...
	if (WriteBuffer(pApplicationSocket, (unsigned char*)&nFileLength, nLength, false, false))
	{
		...
		if ((nFileLength > 0) &&
			!pFiledataSelect.Iterate(pConnection, pApplicationSocket, pSHA256, true, attributes.data(), static_cast<ULONG>(attributes.size())))
		{
			TRACE("MySQL operation failed!\n");
			return false;
		}
 ...
 */
#pragma warning(suppress: 6262)
bool DeleteFile(CWSocket& pApplicationSocket, const std::wstring& strFilePath)
{
	CODBC::CEnvironment pEnvironment;
	CODBC::CConnection pConnection;
