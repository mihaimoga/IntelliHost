...
/**
 * @brief Gets the full path to the IntelliHost XML settings file
 ...
 * =================================
 * 1st choice: %USERPROFILE%\IntelliHost.xml (user-specific settings)
 * 2nd choice: <EXE_DIR>\IntelliHost.xml (fallback if profile unavailable)
 */
const std::wstring GetAppSettingsFilePath()
{
	...
	strFullPath.replace_filename(_T("IntelliHost"));  // Replace exe name with "IntelliHost"
...
/**
 * @brief Loads the service port from the IntelliHost XML settings file
 * @return The service port number, or the default IntelliHostPort on error
 * 
 * XML STRUCTURE:
 * ==============
 * <Settings>
...
{
	int nServicePort = IntelliHostPort;  // Default fallback value
	...
	try {
		...
		// Read service port from [IntelliHost] section
		nServicePort = pAppSettings.GetInt(IntelliHostSection, _T("ServicePort"));
	}
...
/**
 * @brief Saves the service port to the IntelliHost XML settings file
...
bool SaveServicePort(const int nServicePort)
{
	...
	try {
		...
		CXMLAppSettings pAppSettings(GetAppSettingsFilePath(), true, true);
		// Write service port to [IntelliHost] section
		pAppSettings.WriteInt(IntelliHostSection, _T("ServicePort"), nServicePort);
	}
	catch (CAppSettingsException& pException)
	{
...
/**
 * @brief Loads database and server connection settings from the IntelliHost XML file
...
bool LoadAppSettings(std::wstring& strHostName, int& nHostPort, std::wstring& strDatabase, std::wstring& strUsername, std::wstring& strPassword)
{
	...
	try {
		...
		CXMLAppSettings pAppSettings(GetAppSettingsFilePath(), true, true);
		// Read all database connection settings from [IntelliHost] section
		strHostName = pAppSettings.GetString(IntelliHostSection, _T("HostName"));
		nHostPort = pAppSettings.GetInt(IntelliHostSection, _T("HostPort"));
		strDatabase = pAppSettings.GetString(IntelliHostSection, _T("Database"));
		strUsername = pAppSettings.GetString(IntelliHostSection, _T("Username"));
		strPassword = pAppSettings.GetString(IntelliHostSection, _T("Password"));
	}
	catch (CAppSettingsException& pException)
	{
...
/**
 * @brief Saves database and server connection settings to the IntelliHost XML file
...
bool SaveAppSettings(const std::wstring& strHostName, const int nHostPort, const std::wstring& strDatabase, const std::wstring& strUsername, const std::wstring& strPassword)
{
	...
	try {
		// Open or create XML settings file
		CXMLAppSettings pAppSettings(GetAppSettingsFilePath(), true, true);
		// Write all database connection settings to [IntelliHost] section
		// These settings persist across service restarts
		pAppSettings.WriteString(IntelliHostSection, _T("HostName"), strHostName.c_str());
		pAppSettings.WriteInt(IntelliHostSection, _T("HostPort"), nHostPort);
		pAppSettings.WriteString(IntelliHostSection, _T("Database"), strDatabase.c_str());
		pAppSettings.WriteString(IntelliHostSection, _T("Username"), strUsername.c_str());
		pAppSettings.WriteString(IntelliHostSection, _T("Password"), strPassword.c_str());
	}
	catch (CAppSettingsException& pException)
	{
